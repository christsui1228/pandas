...
class CustomerService:
    ...
    def get_customer_summary(self) -> dict[str, int | float]:
        """使用pandas获取客户数据摘要"""
        session = self._get_session()
        
        try:
            # 获取样品客户数据
            sample_query = select(SampleCustomer)
            sample_customers_df = pd.read_sql(sample_query, session.connection())
            
            # 获取批量客户数据
            bulk_query = select(BulkCustomer)
            bulk_customers_df = pd.read_sql(bulk_query, session.connection())
            
            # 默认值，防止空数据集
            if sample_customers_df.empty and bulk_customers_df.empty:
                return {
                    "total_customers": 0,
                    "sample_customers": 0,
                    "bulk_customers": 0,
            ...
            # 计算样品客户数量
            sample_customers_count = len(sample_customers_df) if not sample_customers_df.empty else 0
            
            # 计算批量客户数量
            bulk_customers_count = len(bulk_customers_df) if not bulk_customers_df.empty else 0
            
            # 计算转化客户数量
            converted_count = len(sample_customers_df[sample_customers_df['converted_to_bulk']]) if not sample_customers_df.empty else 0
            
            # 计算总客户数量（注意：由于可能有重复，这可能不是简单的相加）
            total_count = sample_customers_count + bulk_customers_count
            
            # 计算转化率
            conversion_rate = round(converted_count / sample_customers_count * 100, 2) if sample_customers_count > 0 else 0
            
            return {
                "total_customers": total_count,
                "sample_customers": sample_customers_count,
                "bulk_customers": bulk_customers_count,
    ...
    def find_unconverted_customers(self, limit: int = 50) -> list[dict]:
        ...
        try:
            # 查询未转化客户
            customers_query = select(SampleCustomer).where(
                SampleCustomer.converted_to_bulk == False
            ).order_by(SampleCustomer.last_sample_date.desc())
            
            customers_df = pd.read_sql(customers_query, session.connection())
            ...
            sample_orders_query = select(
                SampleOrderCustomer.sample_customer_id,
                SampleOrderCustomer.order_id,
                SampleOrderCustomer.order_date,
                SampleOrderCustomer.amount
            ).where(
                SampleOrderCustomer.sample_customer_id.in_(customer_ids)
            ).order_by(SampleOrderCustomer.order_date.desc())
            
            orders_df = pd.read_sql(sample_orders_query, session.connection())
            ...
            for _, customer_row in customers_df.iterrows():
                ...
                # 获取当前客户的最近5个订单
                customer_orders = orders_df[orders_df['sample_customer_id'] == customer_id].head(5)
