...
async def create_sample_order(order: SampleOrderCreate, db: Session = Depends(get_session)):
    """创建新的样品订单"""
    existing_order_stmt = select(SampleOrder).where(SampleOrder.order_id == order.order_id)
    existing_order = db.exec(existing_order_stmt).first()
    if existing_order:
...
async def update_sample_order(order_id: str, order: SampleOrderUpdate, db: Session = Depends(get_session)):
    """更新现有样品订单"""
    db_order_stmt = select(SampleOrder).where(SampleOrder.order_id == order_id)
    db_order = db.exec(db_order_stmt).first()
    if not db_order:
        raise HTTPException(status_code=404, detail="订单不存在")
